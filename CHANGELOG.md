...
+ [修复] Docker 镜像构建允许按目标架构补齐 Rollup 原生依赖，避免 GitHub Actions 构建失败。
+ [修复] 修复画布本地 Agent 面板事件处理的闭合错误，避免前端构建失败。
+ [修复] 修复画布生成视频后持久化函数未导入导致页面崩溃的问题。
+ [修复] 修复画布视频任务失败与重试所需运行时方法未导入导致页面崩溃，补齐 task ID 持久化与刷新恢复，并增加对应回归测试。
+ [调整] 生图工作台提示词支持通过 `@图片序号` 引用已上传的参考图。
+ [新增] 支持通过 Cloudflare R2 私有 Worker 同步提示词缓存及其封面、参考图片，并保存新生成和上传的图片。
...
+ [新增] GitHub Actions 支持在 main、版本标签和手动触发时构建多架构 GHCR 镜像，并发布 latest、语义版本与提交标签。
+ [新增] 画布图片节点支持在内容参考与风格参考之间切换，并将引用角色传入图片和视频生成提示。
+ [优化] 画布视频任务保存任务 ID 并在刷新后恢复查询，避免已提交任务结果丢失或重复付费。
+ [修复] 修复 WebDAV 删除数据复现、Agent URL 自动连接失效、多标签页串台及付费生成未进入工具确认的问题。
+ [修复] 画布与素材导入增加异步解压、体积限制和结构校验，并补齐快速关闭保存与媒体 URL 清理。
+ [修复] 修复合并后 Bun 锁文件缺失和插件视频任务恢复类型不完整，避免镜像构建失败与画布视频状态异常。
