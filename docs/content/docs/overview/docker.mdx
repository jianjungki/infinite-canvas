...
主应用镜像发布在 `ghcr.io/basketikun/infinite-canvas`，同时支持 `linux/amd64` 和 `linux/arm64`。`main` 分支更新、推送 `v*` 版本标签或在 GitHub Actions 中手动运行 **Build Docker image** 都会构建镜像。
...
手动运行 Action 时可以填写额外标签，例如 `staging`，也可以选择是否更新 `latest`。
...
仓库根目录的 `docker-compose.yml` 默认使用 `latest`。后续更新服务时执行：
...
```yaml
image: ghcr.io/basketikun/infinite-canvas:0.6.1
```
